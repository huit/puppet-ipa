...
define ipa::clientinstall (
  ...
  $clientinstallcmd = shellquote('/usr/sbin/ipa-client-install',"--server=freeipa-${master_region}-${domain}","--hostname=${host}","--domain=${domain}","--realm=${realm}","--password=${otp}",$mkhomediropt,$ntpopt,$fixedprimaryopt,'--unattended')
